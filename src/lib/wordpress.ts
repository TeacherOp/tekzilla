  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        if (
          (response.status === 503 || response.status === 507) &&
          attempt < retries
        ) {
...
export async function getAllCategoriesPaginated(): Promise<Category[]> {
  let allCategories: Category[] = [];
  let page = 1;
  let hasMore = true;

  while (hasMore) {
    const res = await wordpressFetchWithPagination<Category[]>(
      "/wp-json/wp/v2/categories",
      { per_page: 20, page },
    );
    allCategories.push(...res.data);
    hasMore = page < res.headers.totalPages;
    page++;
  }
  return allCategories;
}
export async function getAllPostsPaginated(filterParams?: {
  author?: string;
  tag?: string;
  category?: string;
  search?: string;
  ...
  while (hasMore) {
    ...
    const res = await wordpressFetchWithPagination<Post[]>(
      "/wp-json/wp/v2/posts",
      query,
    );
...
// Fetch most popular tags, default 45, sorted by count descending
export async function getPopularTags(limit = 45): Promise<Tag[]> {
  return wordpressFetch<Tag[]>("/wp-json/wp/v2/tags", {
    per_page: limit,
    orderby: "count",
    order: "desc",
